...
## Reference existing AI Foundry resource (deployed manually by school)
##
data "azurerm_cognitive_account" "ai_foundry" {
  name                = var.azure_openai_resource_name
  resource_group_name = var.azure_openai_resource_group
}

# Grant App Service Access to AI Foundry (Managed Identity)
# School should manually grant this permission via Azure Portal
resource "azurerm_role_assignment" "webapp_ai" {
  count = var.auto_grant_ai_access ? 1 : 0
  
  scope                = data.azurerm_cognitive_account.ai_foundry.id
